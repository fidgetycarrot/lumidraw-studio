...
As of **v0.14.0**, the Generate tab also has a separate **workspace / draft**
area: you can temporarily swap model, sampler, steps, CFG, size, and LoRAs for
manual generations without immediately changing the preset your chat relies on.
...
- **Generate tab** — the top preset selector is still the chat's committed
  recipe, but the **Workspace / draft settings** section is your sandbox. Load
  a preset or sync from Draw Things, tweak model / sampler / steps / CFG / size
  / LoRAs, then generate manually without changing the preset until you click
  **Update active preset** or **Save as new preset**.
- Prompt, optional seed (blank = random, `↩ last` reuses the previous seed),
  Generate. Results land in the panel history and in Lumiverse's image
  library.
